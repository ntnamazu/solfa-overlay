...
afterEach(async () => {
  // 予約済みの自動保存を残したままディレクトリを消すと、後続テストの実行中にタイマーが
  // 発火し「消えた一時ディレクトリへ書けない」というエラーログだけが残る。テストが
  // デバウンス（既定 300ms）より短く終わるようになって初めて起きるようになった。
  // ここで書き切らせて予約を空にする（ディレクトリごと捨てるため保存内容は問わない）。
  // 保存の失敗そのものは各テストの検証対象であり、片付けでは握りつぶす
  await session.flushPendingSave().catch(() => undefined);
