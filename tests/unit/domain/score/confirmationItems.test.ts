...
describe('buildConfirmationItems', () => {
  ...
  describe('並び順（不一致の多い譜表を先頭に出す）', () => {
    const artifacts = artifactsOf([
      [[staff('P1', 'TREBLE'), staff('P2', 'ALTO'), staff('P3', 'BASS')]],
    ]);

    it('クロスチェック不一致の多いグループが先頭に来る', () => {
      const issues = [
...
describe('訂正しても並び順が動かない', () => {
  ...
  it('訂正して不一致が消えたグループも、前回と同じ位置に留まる', () => {
    const initial = buildConfirmationItems(artifacts, issues);
    expect(initial.map((item) => item.partId)).toEqual(['P6', 'P4', 'P7']);

    // 先頭（P6）を訂正した結果、その譜表の不一致が解消された状態を再現する
    ...
    // 素の生成では不一致 0 件の P6 が末尾へ落ちる
    expect(rebuilt.map((item) => item.partId)).toEqual(['P4', 'P7', 'P6']);

    // 直した行が目の前から動くと「上から順に直す」導線が破綻するため、前回の並びを維持する
    const merged = mergeCorrections(rebuilt, corrected);
    expect(merged.map((item) => item.partId)).toEqual(['P6', 'P4', 'P7']);
    expect(merged[0]?.corrected).toBe('TREBLE');
    expect(merged[0]?.mismatchCount).toBe(0);
  });

  it('前回になかった項目は末尾に付く（並びの既知部分を崩さない）', () => {
    const previous = buildConfirmationItems(artifacts, issues).slice(1); // P6 を知らない状態
    const merged = mergeCorrections(buildConfirmationItems(artifacts, issues), previous);
    expect(merged.map((item) => item.partId)).toEqual(['P4', 'P7', 'P6']);
  });

  it('前回が空なら不一致件数の降順のまま（初回の並びは影響を受けない）', () => {
