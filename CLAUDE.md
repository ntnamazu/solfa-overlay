...
- 開発環境: devcontainer（Node.js v20.x / npm 10.x）
  - コンテナの Node.js はツール実行用（テスト・ビルド・リント等）。アプリのランタイムとは独立に管理し、必ずしもバージョンを一致させない
- アプリ本体: Electron + TypeScript 5.x（strict モード必須）
  - 実行時の Node.js バージョンは Electron 内蔵ランタイムに従う
- OMR: Audiveris 5.x（JRE 21 同梱・ヘッドレス実行）

**詳細な技術スタック・バージョン・選定理由は [docs/architecture.md](docs/architecture.md) を正とする。CLAUDE.md には転記しない（二重管理防止）。**
