...
5. **階名を確認して書き出す** — 付与された階名を楽譜の上で確認します。間違った階名は押して書き換え・削除でき、階名が付かなかった小節（黄色の枠）には押した位置に階名を書き足せます。最後に「注釈付きPDFを出力」で階名入りのPDFを出力します。
...
- [x] 注釈の手動編集（Editor画面の楽譜プレビューで階名を押して書き換え・削除、階名の無い位置に書き足し。編集はプロジェクトに保存され、再解析後も保たれる）
- [ ] **転調点の指定**（現在は調号の変わる位置のみ自動で判定します）
