...
class AnnotationManager {
  ...
  // 以下は F-5 の編集操作の部品。Editor からは applyAnnotationEdit（純関数）経由で使う
...
// Editor の楽譜プレビューからの編集 1 件（追加・書き換え・削除・削除の取り消し）を適用する
applyAnnotationEdit(
  annotations: readonly Annotation[],
  edit: AnnotationEdit,
  context: { pages: readonly PageInfo[]; settings: ProjectSettings; newId: () => string },
): Annotation[]; // 適用できない編集は AnnotationEditError
```
...
- **文言の書き方**（指示は「〜してください」・見出しは名詞句・評価を書かない等）は
  [開発ガイドライン](development-guidelines.md)「UIテキスト規約」を正とする。
  ここには「何を語彙として選ぶか」だけを書く
...
| 区画                         | 説明                                                                                                 |
| ---------------------------- | ---------------------------------------------------------------------------------------------------- |
| 直すところ                   | 訂正できる issue（`systemMeasureCountMismatch`）のみ。段の実際の小節数を入力する                     |
...
1. 「直すところ」の各行で、その段に実際いくつ小節があるかを楽譜と見比べて入力する
...
| 階名の表記           | 音節体系（コダーイ式 / Tonic sol-fa 略記）・短調の基準（La基準 / Do基準）の切り替え。選ぶと即座に再解析し、下の階名プレビューに反映する（F-3） |
| 階名プレビュー       | パート×小節の階名列（曲の先頭部分のみ）。#4 以降は「先頭部分の階名を文字で見る」として畳んで残す                                               |
| スキップ小節一覧     | 階名が欠落している小節。#4 以降は各行から楽譜上の位置へジャンプできる                                                                          |
| 配置警告             | 衝突回避に失敗した注釈の件数                                                                                                                   |
...
| 項目         | 説明                                  | フォーマット                             | 状態           |
| ------------ | ------------------------------------- | ---------------------------------------- | -------------- |
| 楽譜ページ   | 元PDFのレンダリング＋注釈オーバーレイ | PDF.js キャンバス＋SVG                   | 実装済み（#4） |
| 階名注釈     | 幹音/半音変化を色分け                 | 幹音=濃赤・変化音=紫＋太字（設定変更可） | 実装済み（#4） |
| スキップ小節 | 楽譜上のハイライト                    | 黄ハイライト。一覧のクリックでジャンプ   | 実装済み（#4） |
| 転調点       | KeyRegion の境界                      | 小節上のマーカー（auto=グレー、user=青） | 未実装（#6）   |
| 配置警告     | 衝突回避失敗の位置                    | 楽譜上の警告アイコン（「!」）            | 実装済み（#4） |
| 注釈の編集   | 追加・書き換え・削除                  | 楽譜上の注釈・位置をクリックして編集     | 実装済み（#5） |

Phase 5 のテキスト表示（階名プレビュー表）は、楽譜を表示できない場合の代わりとして
「先頭部分の階名を文字で見る」（既定は畳む）に残している。
...
- **何をどこに描くかは Main（`domain/render/scorePreview.ts` の `buildScorePreview`）で確定させる**。
  注釈の文字列・変化音の判定（`annotationContent`）、描けない文字の置換（`fontMetrics.displayText`）、
  座標の縮尺（`toPreviewPoint`）を OverlayRenderer と共有し、**出力PDFと同じ位置・同じ文字**になることを構造で担保する。
  書体（未知の名前→sans-serif）と色（解釈できない値→既定色）も出力PDFと同じ落とし先へ正規化して渡す
- 座標は **pt・左上原点**で渡す。SVG の `viewBox` をページのポイント寸法にそろえると、SVG の `<text x y>` は
  pdf-lib の `drawText` と同じく「左端＋ベースライン」を基準にするため、`Annotation.anchor` を変換せずに使える
- ページは**元PDFのページ**でまとめる（Victoria は Audiveris 4 ページ → 元PDF 3 ページ）
- スキップ小節の枠は「小節の stack の横範囲 × そのパートの譜表の譜線範囲（上下に半譜線間隔の余白）」。
  譜線範囲は sheet XML の `<staff><lines><line><point>` から読む（`OmrStaff.extent`）。stack が無い小節は段全体の横範囲へ広げ、
  段や譜線が分からない小節は枠を作らない（一覧には「楽譜上の位置を特定できません」と添えて残す）
- 元PDF のバイト列は `ProjectSnapshot` に入れず、専用の IPC（`project:getSourcePdf`。パスは受け取らない）で
  **プロジェクトごとに 1 回だけ**取得する。訂正や表記の切り替えでは重ね描きの内容だけが届き直す
- Renderer はページ枠を先に並べ、**画面の近く（前後おおむね 1 画面）のページだけ**キャンバスと注釈テキストを描く
  （IntersectionObserver）。スキップ小節の枠と警告は件数が少なくジャンプ先になるため常に描く
- 実測（`tests/integration/pipeline/score-preview.test.ts`）: 画面に重ねる注釈数は出力PDFの描画件数と一致
  （Victoria 808 / divisi 3500）。スキップ小節は全件が位置を持つ（Victoria 1 / divisi 135）。配置警告 Victoria 0 / divisi 29

**Editor の区画の並び（#17 で決定）**:

概要 → 階名が付かなかった小節 → 配置警告 → 孤立注釈 → ページの問題 → 適用されなかった訂正 →
PDF出力 → 確認画面へ戻る → 階名の表記 → **楽譜プレビュー（最後）**。

- 楽譜プレビューは全ページを縦に並べるため、後ろに置いた区画は画面のずっと下に隠れる。
  h1 直下の案内文が指す「注釈付きPDFを出力」と、ジャンプの起点になるスキップ小節の一覧をプレビューより上に置く
- 「階名の表記」は切り替えた結果がすぐ下で見えるよう、プレビューの直前に置く
- 検討した別案: プレビューを固定の高さの枠にして中だけスクロールさせる / 左右 2 カラム（表示倍率の改善と合わせて再検討する）

### 注釈の編集の操作フロー（F-5・#5 で実装）

1. 楽譜プレビューの階名を押すと選択され、画面下端に固定した編集パネルが開く
   （プレビューは縦に長く、区画の頭に置くと押した位置から見えないため）
2. 自動で付いた階名は「書き換える」「削除」、書き換え済みなら「自動の階名に戻す」ができる。書き足した階名は「書き換える」「削除」
3. 階名の無い位置（スキップ小節の黄色い枠の中を含む）を押すと、そこへ書き足すパネルが開く。**押した点が書き足す文字の中心**になる
4. 削除した直後は「元に戻す」で取り消せる（自動の階名は `deleted` の印を外し、書き足した階名は削除前の内容をそのまま戻す）
5. 確定した編集は即座に楽譜プレビューへ反映され、自動保存される。出力済みの結果表示は消す（出力済みのPDFには編集が入っていないため）

**設計の要点**:

- Renderer は `.omr` の座標系も Audiveris のページも知らない。IPC（`project:editAnnotation`）では
  **元PDFのページ＋プレビュー座標（pt・左上原点）**で意図だけを送り、Main（`applyAnnotationEdit` / `manualAnchorAt`）が
  `fromPreviewPoint`（`toPreviewPoint` の逆。縮尺の式を共有）で `.omr` px へ戻す。1 つの元PDFページに複数の Audiveris ページが
  対応するときは最小の `pageIndex` を選ぶ（同じ sheet の Audiveris ページは画像座標系を共有するため、描かれる位置は変わらない）
- 編集のたびに**解析を流し直す**（`analyzeAndSave`）。書き足した手動注釈を自動注釈が避ける配置と、
  プレビュー・出力PDF・再読み込み後の結果が同じ経路で決まり、「画面と再読み込み後で位置が違う」が起きない
- 注釈の編集は承認（`completedAt`）を無効化しない。音部記号・調の確認の対象を変えないため（設定変更と同じ扱い）
- 手動注釈の id は `manual-<UUID>`（自動注釈の `solfa-<noteId>` と衝突しない）
- 文字は前後の空白を落とし、空・16 文字超は受け付けない（`MANUAL_TEXT_MAX_LENGTH`。Renderer の入力欄と Main の検証で共有）。
  標準フォントで描けない文字（全角など）は出力PDFと同じく `?` へ置き換わるため、パネルで半角の英字を案内する
- クリックは SVG の 1 か所で受け、押した要素の `data-annotation` で階名を判別する（注釈は 1 ページ数百あるため要素ごとにハンドラを付けない）

**既知の限界**:

- 手動注釈は幹音の色（濃赤・通常体）で描く。音符を参照しないため変化音かどうかが決まらない（指定するにはスキーマ変更が要る）
- 位置の移動（ドラッグ）はできない。削除して書き足す
- 階名の選択はマウス操作のみ（1 ページ数百の階名をすべてタブ移動の対象にすると、キーボード操作がかえって成立しない）
